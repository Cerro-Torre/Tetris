...
void print_stats(game_stats_t *stats) {
  MVPRINTW(2, GAME_SCREEN_WIDTH + 12, "%d", stats->level);
  MVPRINTW(5, GAME_SCREEN_WIDTH + 12, "%d", stats->score);
  MVPRINTW(8, GAME_SCREEN_WIDTH + 12, "%d", stats->speed);
  MVPRINTW(11, GAME_SCREEN_WIDTH + 12, "%d", stats->lives);
}
...
void print_cars(board_t *game) {
  for (int i = MAP_PADDING + 1; i < GAME_SCREEN_HEIGHT - MAP_PADDING + 1; i++) {
    if (i % 2 == (MAP_PADDING + 1) % 2) {
      for (int j = 1; j < GAME_SCREEN_WIDTH + 1; j++) MVADDCH(i, j, ACS_BLOCK);
    } else {
